...
pub struct NucleoEntry {
    pub desktop_entry: DesktopEntry,
    pub id: u64,
}

impl AsRef<str> for NucleoEntry {
    fn as_ref(&self) -> &str {
        self.desktop_entry.name.as_ref()
    }
}
...
pub fn scrubber(config: &Config) -> Result<Vec<NucleoEntry>, Box<dyn std::error::Error>> {
